...
{#snippet payment(payment, type, month, year, payerUuid)}
	{#if payment}
		<abbr class="td payment ok flex-1 last:rounded-r-2xl border-none! w-full! md:max-w-1/4! md:w-1/4" title={payment.comment}
		      style="font-style: unset; text-decoration: unset">
			<form action="?/deletePayment" class="flex-1 *:bg-(--input)! bg-(--input) shadow-md shadow-slate-950/40 rounded-2xl"
	...
	{:else}
		<div class="td payment bad last:rounded-r-2xl! border-none! w-full! md:max-w-1/4! mdLw-1/4">
			<form action="?/addPayment" method="POST" class="flex-1 bg-(--link)! *:bg-(--link)! outline-0! *:outline-0! border-0 b*:border-0! shadow-md shadow-slate-950/40" use:enhance>
           ...
           peer-checked:pointer-events-auto"
					>
						<div
							class="bg-(--background-secondary)! absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-[-200%] flex flex-col gap-5 p-2 md:w-1/4!:w-full! rounded-md! h-fit shadow-md shadow-slate-950/40">
                                <textarea name="comment"
                                          class="bg-(--input) text-(--text-secondary) block rounded-md p-2! h-20! shadow-md shadow-slate-950/40">{payment?.comment}</textarea>
							<label for={`show-comment-${payerUuid}-${month}-${year}`}
							       class="block text-gray-200 bg-(--click) rounded-sm text-2xl cursor-pointer p-1! hover:bg-neutral-800 duration-200 shadow-md shadow-slate-950/40">Zapisz</label>
						</div>
					</div>
				</label>
...
<div class="bg-(--background-primary) shadow-md shadow-slate-950/20 rounded-2xl p-4">
	<a href={resolve('/paymentHistory')}
	   class="absolute top-16 left-4 p-3 rounded-xl bg-neutral-200 border-2 border-neutral-400 text-neutral-400 hover:text-neutral-600 hover:text-shadow-2 hover:text-shadow-black/20 duration-200 desktop">Szczegóły</a>
	<div id="filterHolder" class="bg-(--background-secondary) text-(--text-primary-dark) shadow-md shadow-slate-950/20">
		<span>Znajdź:</span>
		<input type="text" bind:value={memberTextFilter} class="input bg-(--input) text-(--text-primary)!" />
	...
	<div class="tabletop desktop">
		<div class="thead">
			<div class="tr *:bg-(--background-secondary) *:p-2! *:align-middle text-(--text-primary-dark) shadow-md shadow-slate-950/20 rounded-2xl">
